 ...
 * @param {string} fullName - Nombre completo del ganador
 * @returns {string} - Solo el primer nombre
 */
const getFirstNameOnly = (fullName) => {
  if (!fullName || typeof fullName !== "string") return "Ganador anónimo"

  // Limpiar espacios extras y dividir por espacios
  const nameParts = fullName.trim().split(/\s+/)

  // Retornar solo el primer nombre
  return nameParts[0] || "Ganador anónimo"
}
...
export const publicAPI = {
    ...
    try {
      ...
      const normalizedData = {
        results: results.map((roulette) => ({
          ...
          participants_count: Number.parseInt(roulette.participants_count) || 0,
          prizes_count: Number.parseInt(roulette.prizes_count) || 0,
  ...
  getPublicRouletteDetail: async (rouletteId) => {
    try {
      ...
      const normalizedData = {
        ...
        participants_count: Number.parseInt(data.participants_count) || 0,
        ...
        prizes: Array.isArray(data.prizes)
          ? data.prizes.map((prize) => ({
              ...
              display_order: Number.parseInt(prize.display_order) || 0,
            }))
          : [],
        ...
        settings: data.settings
          ? {
              max_participants: Number.parseInt(data.settings.max_participants) || 0,
    ...
    try {
      ...
      const normalizedData = {
        results: results.map((item) => ({
          ...
          participants_count: Number.parseInt(item.participants_count) || 0,
          roulette_name: item.roulette_name || "Sorteo",
          winner_name: getFirstNameOnly(item.winner_name),
        })),
        pagination: data.pagination || {
          count: data.count || results.length,
          next: data.next || null,
  ...
  getPublicMetrics: async () => {
    try {
      ...
      const normalizedMetrics = {
        roulettes_total: Number.parseInt(data.roulettes_total) || 0,
        active_roulettes: Number.parseInt(data.active_roulettes) || 0,
        winners_total: Number.parseInt(data.winners_total) || 0,
        participants_total: Number.parseInt(data.participants_total) || 0,
  ...
  getHomeData: async () => {
    try {
      ...
      const roulettes = roulettesResult.status === "fulfilled" ? roulettesResult.value?.results || [] : []

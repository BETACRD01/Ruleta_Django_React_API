...
import {
  ...
  Trophy, Calendar, FileText, ChevronDown, ChevronUp,
...
const ParticipantManager = ({ onRefreshDashboard }) => {
  ...
  const loadData = async () => {
    try {
      ...
      // Cargar todas las participaciones (esto podría optimizarse con paginación)
        ...
        try {
          ...
          if (participantsResponse.success && participantsResponse.participants) {
            const enrichedParticipants = participantsResponse.participants.map(p => ({
              ...
              // Simular fecha de participación (en un caso real vendría del backend)
  ...
  const applyFilters = () => {
    ...
    // Filtro por ruleta
    if (filters.roulette) {
      filtered = filtered.filter(p => p.roulette_id === parseInt(filters.roulette));
    }
  ...
  const handleBulkAction = async (action) => {
    ...
    try {
      ...
      // Aquí implementarías las acciones en lote
      // Por ahora solo simulamos
      console.log(`Ejecutando acción ${action} en participantes:`, selectedParticipants);
